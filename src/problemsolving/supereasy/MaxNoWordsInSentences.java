...
public class MaxNoWordsInSentences {
    static void main() {
        ...
        System.out.println(solution(sentenceArr2));
    }
    
    static int solution(String[] sentences) {
        
        Optional<Integer> maxByStream = Arrays.stream(sentences)
                                      .map(sen -> sen.split(" "))
                                      .map(e -> e.length)
                                      .max(Integer::compareTo);
        return maxByStream.get();
    }
